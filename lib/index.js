...
class AnyList extends EventEmitter {
   ...
   * @return {Promise<RecipeCollection[]>} recipeCollections
   */
	async getRecipeCollections(refreshCache = true) {
		const decoded = await this._getUserData(refreshCache);

		this.recipeCollections = decoded.recipeDataResponse.recipeCollections.map(collection => new RecipeCollection(collection, this));
   ...
   * @param {string} name recipe collection name
   * @return {RecipeCollection} recipeCollection
   */
   ...
   */
	...
	async _getUserData(refreshCache) {
		if (!this._userData || refreshCache) {
			...
			if (!this.uid) {
				// Try linkedUsers first (works for shared accounts)
				if (this._userData.recipeDataResponse.linkedUsers) {
					const currentUser = this._userData.recipeDataResponse.linkedUsers
				...
				// Fall back to extracting userId from shopping list items
				// (linkedUsers may not include the account owner)
				if (!this.uid && this._userData.shoppingListsResponse?.newLists?.length > 0) {
					for (const list of this._userData.shoppingListsResponse.newLists) {
						if (list.items?.length > 0) {
							this.uid = list.items[0].userId;
							break;
						}
					}
				}

				// Final fallback: use the list creator field (works even with empty lists)
				if (!this.uid && this._userData.shoppingListsResponse?.newLists?.length > 0) {
					this.uid = this._userData.shoppingListsResponse.newLists[0].creator;
				}
			}
		}
